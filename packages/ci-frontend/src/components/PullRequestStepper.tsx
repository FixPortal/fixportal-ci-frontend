...
export function PullRequestStepper({ prs, onClose }: { prs: OpenPr[]; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (el && document.body.contains(el) && !('disabled' in el && (el as HTMLElement & { disabled?: boolean }).disabled)) {
        el.focus()
      } else {
        (document.querySelector('.dashboard-page') as HTMLElement | null)?.focus({ preventScroll: true })
      }
      dlg?.close()
    }
  }, [])
    ...
    >
      ...
      <div className={`pr-card pr-card--${prAgeTone(pr.createdAt)}${pr.isDraft ? ' pr-card--draft' : ''}`}>
        ...
        <div className="pr-card__foot">
          ...
          {prHref !== '#' ? (
            <a className="pr-card__gh" href={prHref} target="_blank" rel="noopener noreferrer">Open on GitHub ↗</a>
          ) : (
